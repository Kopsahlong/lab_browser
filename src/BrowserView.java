...
public class BrowserView {
	...
	private Node makePreferencesPanel() {
		...
		result.getChildren().add(makeButton("SetHomeCommand", "setHomeAndEnableButtons"));
		return result;
	}
	
	public void setHomeAndEnableButtons(){
		myModel.setHome();
		enableButtons();
	}
	// makes a button using either an image or a label
	/*
	 * private Button makeButton (String property, EventHandler<ActionEvent>
	 * handler) { // represent all supported image suffixes final String
	 * IMAGEFILE_SUFFIXES = String.format(".*\\.(%s)", String.join("|",
	 * ImageIO.getReaderFileSuffixes()));
	 * 
	 * Button result = new Button(); String label =
	 * myResources.getString(property); if (label.matches(IMAGEFILE_SUFFIXES)) {
	 * result.setGraphic(new ImageView( new
	 * Image(getClass().getResourceAsStream(DEFAULT_RESOURCE_PACKAGE +
	 * label)))); } else { result.setText(label); } result.setOnAction(handler);
	 * return result; }
	 */
